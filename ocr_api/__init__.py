...
def create_app(test_config=None):
    ...
    def template_ocr_result(ocr_type):
        if request.method == 'POST':
            ...
            print("asdfasd",len(template_information['image']))
    ...
    def template_ocr_excel():
        if request.method == 'POST':
            ...
            with pd.ExcelWriter(f_path+'/template_excel.xlsx', mode='w', engine='xlsxwriter') as writer:
    ...
    # # 문단별 ocr
    # @app.route('/normal-para-ocr', methods=('GET', 'POST', 'OPTION'))
    ...
    #         if not os.path.isdir(app.config['IMAGE_PATH']+'/para_origin'):
    #             os.mkdir(app.config['IMAGE_PATH']+'/para_origin')

    #         f.save(os.path.join(
    #             app.config['IMAGE_PATH']+'/para_origin', filename))
    ...
    #             if not os.path.exists(app.config['IMAGE_PATH']+'/para'):
    #                 os.mkdir(app.config['IMAGE_PATH']+'/para')

    #             img_path = app.config['IMAGE_PATH'] + \
    #                 "/para/"+"data"+str(imgNum) + ".png"
    #             check = cv2.imwrite(img_path, crop_img)

    #             if check:
    #                 img = cv2.imread(img_path)
    #                 send_result.append(pytesseract.image_to_string(
    #                     img, lang='eng+kor_new', config='--oem 3 --psm 6'))

    #                 with open(img_path, 'rb') as im:
    #                     send_image.append(base64.b64encode(
    #                         im.read()).decode('utf8'))
    ...
    #         send_json_file["image"] = send_image
    #         send_json_file["result"] = send_result
    #         # print(send_json_file)
    #         return json.dumps(send_json_file)
